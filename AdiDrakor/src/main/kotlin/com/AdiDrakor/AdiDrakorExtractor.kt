...
object AdiDrakorExtractor : AdiDrakor() {
    ...
    suspend fun invokeGomovies(
        ...
        invokeGpress(
            ...
            episode,
            callback,
            DomainManager.gomoviesAPI, // Menggunakan DomainManager
            "Gomovies",
    ...
    suspend fun invokeIdlix(
        ...
        // Menggunakan DomainManager
        val url = if (season == null) {
            "${DomainManager.idlixAPI}/movie/$fixTitle-$year"
        } else {
            "${DomainManager.idlixAPI}/episode/$fixTitle-season-$season-episode-$episode"
        }
    ...
    suspend fun invokeVidsrccc(
        ...
        try {
            // Menggunakan DomainManager
            val url = if (season == null) {
                "${DomainManager.vidsrcccAPI}/v2/embed/movie/$tmdbId"
            } else {
                "${DomainManager.vidsrcccAPI}/v2/embed/tv/$tmdbId/$season/$episode"
            }
            ...
            val serverUrl = if (season == null) {
                "${DomainManager.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=movie&v=$v&vrf=$vrf&imdbId=$imdbId"
            } else {
                "${DomainManager.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=tv&v=$v&vrf=$vrf&imdbId=$imdbId&season=$season&episode=$episode"
            }

            app.get(serverUrl).parsedSafe<VidsrcccResponse>()?.data?.amap {
                val sources =
                    app.get("${DomainManager.vidsrcccAPI}/api/source/${it.hash}").parsedSafe<VidsrcccResult>()?.data
                        ?: return@amap
                ...
                when {
                    it.name.equals("VidPlay") -> {
                        ...
                        callback.invoke(
                            newExtractorLink(
                                ...
                                this.referer = "${DomainManager.vidsrcccAPI}/"
                            }
                        )
                    ...
                    it.name.equals("UpCloud") -> {
                        val scriptData = app.get(
                            sources.source ?: return@amap,
                            referer = "${DomainManager.vidsrcccAPI}/"
    ...
    suspend fun invokeVidsrc(
        ...
        try {
            ...
            // Menggunakan DomainManager
            val url = if (season == null) {
                "${DomainManager.vidSrcAPI}/embed/movie?imdb=$imdbId"
            } else {
                "${DomainManager.vidSrcAPI}/embed/tv?imdb=$imdbId&season=$season&episode=$episode"
            }
        ...
        runAllAsync(
            {
                // Menggunakan DomainManager
                val url = if (season == null) {
                    "${DomainManager.xprimeAPI}/${servers.first()}?id=$tmdbId"
                } else {
                    "${DomainManager.xprimeAPI}/${servers.first()}?id=$tmdbId&season=$season&episode=$episode"
                }
            ...
            },
            {
                // Menggunakan DomainManager
                val url = if (season == null) {
                    "${DomainManager.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year"
                } else {
                    "${DomainManager.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year&season=$season&episode=$episode"
                }
    ...
    suspend fun invokeWatchsomuch(
        ...
        try {
            ...
            // Menggunakan DomainManager
            val epsId = app.post(
                "${DomainManager.watchSomuchAPI}/Watch/ajMovieTorrents.aspx", data = mapOf(
                    "index" to "0",
                    "mid" to "$id",
            ...
            val subUrl = if (season == null) {
                "${DomainManager.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part="
            } else {
                "${DomainManager.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part=S${seasonSlug}E${episodeSlug}"
            }
            ...
            app.get(subUrl).parsedSafe<WatchsomuchSubResponses>()?.subtitles?.map { sub ->
                subtitleCallback.invoke(
                    newSubtitleFile(
                        ...
                        fixUrl(sub.url ?: return@map null, DomainManager.watchSomuchAPI)
                    )
                )
            }
        } catch (e: Exception) {
    ...
    suspend fun invokeMapple(
        ...
        try {
            ...
            // Menggunakan DomainManager
            val url = if (season == null) {
                "${DomainManager.mappleAPI}/watch/$mediaType/$tmdbId"
            } else {
                "${DomainManager.mappleAPI}/watch/$mediaType/$season-$episode/$tmdbId"
            }
            ...
            callback.invoke(
                newExtractorLink(
                    ...
                    this.referer = "${DomainManager.mappleAPI}/"
                    this.headers = mapOf(
                        "Accept" to "*/*"
                    )
                }
            ...
            val subRes = app.get(
                "${DomainManager.mappleAPI}/api/subtitles?id=$tmdbId&mediaType=$mediaType${if (season == null) "" else "&season=1&episode=1"}",
                referer = "${DomainManager.mappleAPI}/"
            ).text
            tryParseJson<ArrayList<MappleSubtitle>>(subRes)?.map { subtitle ->
                subtitleCallback.invoke(
                    newSubtitleFile(
                        subtitle.display ?: "",
                        fixUrl(subtitle.url ?: return@map, DomainManager.mappleAPI)
                    )
                )
            }
        } catch (e: Exception) {
    ...
    suspend fun invokeVidlink(
        ...
        try {
            ...
            // Menggunakan DomainManager
            val url = if (season == null) {
                "${DomainManager.vidlinkAPI}/$type/$tmdbId"
            } else {
                "${DomainManager.vidlinkAPI}/$type/$tmdbId/$season/$episode"
            }

            val videoLink = app.get(
                url, interceptor = WebViewResolver(
                    Regex("""${DomainManager.vidlinkAPI}/api/b/$type/A{32}"""), timeout = 15_000L
                )
            ...
            callback.invoke(
                newExtractorLink(
                    ...
                    this.referer = "${DomainManager.vidlinkAPI}/"
                }
            )
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    suspend fun invokeVidfast(
        ...
        try {
            ...
            // Menggunakan DomainManager
            val url = if (season == null) {
                "${DomainManager.vidfastAPI}/$type/$tmdbId"
            } else {
                "${DomainManager.vidfastAPI}/$type/$tmdbId/$season/$episode"
            }

            val res = app.get(
                url, interceptor = WebViewResolver(
                    Regex("""${DomainManager.vidfastAPI}/$module/JEwECseLZdY"""),
                    timeout = 15_000L
                )
            ).text
            ...
            tryParseJson<ArrayList<VidFastServers>>(res)?.filter { it.description?.contains("Original audio") == true }
                ?.amapIndexed { index, server ->
                    val source =
                        app.get("${DomainManager.vidfastAPI}/$module/Sdoi/${server.data}", referer = "${DomainManager.vidfastAPI}/")
    ...
    suspend fun invokeWyzie(
        ...
        try {
            // Menggunakan DomainManager
            val url = if (season == null) {
                "${DomainManager.wyzieAPI}/search?id=$tmdbId"
            } else {
                "${DomainManager.wyzieAPI}/search?id=$tmdbId&season=$season&episode=$episode"
            }
    ...
    suspend fun invokeVixsrc(
        ...
        try {
            ...
            // Menggunakan DomainManager
            val url = if (season == null) {
                "${DomainManager.vixsrcAPI}/$type/$tmdbId"
            } else {
                "${DomainManager.vixsrcAPI}/$type/$tmdbId/$season/$episode"
            }
            ...
            listOf(
                ...
                VixsrcSource("Vixsrc [Beta]", video2, "${DomainManager.mappleAPI}/"),
            ).map {
                callback.invoke(
                    newExtractorLink(
                        it.name,
        ...
        try {
            ...
            callback.invoke(
                newExtractorLink(
                    ...
                    this.referer = "${DomainManager.vidsrccxAPI}/"
                    this.headers = mapOf(
                        "Accept" to "*/*"
                    )
                }
    ...
    suspend fun invokeSuperembed(
        ...
        try {
            ...
            // Menggunakan DomainManager
            val token = app.get("${DomainManager.superembedAPI}/directstream.php?video_id=$tmdbId&tmdb=1$path").url.substringAfter(
                "?play="
            )
    ...
    suspend fun invokeVidrock(
        ...
        try {
            val type = if (season == null) "movie" else "tv"
            // Menggunakan DomainManager
            val url = "${DomainManager.vidrockAPI}/$type/$tmdbId${if (type == "movie") "" else "/$season/$episode"}"
            val encryptData = VidrockHelper.encrypt(tmdbId, type, season, episode)

            app.get("${DomainManager.vidrockAPI}/api/$type/$encryptData", referer = url).parsedSafe<LinkedHashMap<String, HashMap<String, String>>>()
                ?.map { source ->
                    if (source.key == "source2") {
                        val json = app.get(source.value["url"] ?: return@map, referer = "${DomainManager.vidrockAPI}/").text
                        tryParseJson<ArrayList<VidrockSource>>(json)?.reversed()?.map mirror@{ it ->
                            callback.invoke(
                                newExtractorLink(
                                    ...
                                    this.headers = mapOf(
                                        "Range" to "bytes=0-",
                                        "Referer" to "${DomainManager.vidrockAPI}/"
                                    )
                                }
                            )
                        }
                        ...
                        callback.invoke(
                            newExtractorLink(
                                ...
                                this.referer = "${DomainManager.vidrockAPI}/"
                                this.headers = mapOf(
                                    "Origin" to DomainManager.vidrockAPI
                                )
                            }
                        )
                    }
