...
class DramaDrip : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = document.selectFirst("div.wp-block-column > h2.wp-block-heading")?.text()
            ?.substringBefore("(")?.trim().toString() ?: "Unknown Title"
        ...
        // Mengambil Metadata dari Cinemeta (Stremio)
        ...
        if (tvType == TvType.TvSeries) {
            ...
            val finalEpisodes = tvSeriesEpisodes.map { (seasonEpisode, links) ->
                ...
                // PERUBAHAN UTAMA: Bungkus LinkData agar Wyzie bisa pakai ID TMDB
            ...
            // PERUBAHAN UTAMA: Bungkus LinkData untuk Movie
            val linkData = LinkData(
                links = hrefs,
                tmdbId = tmdbId,
                season = null,
    ...
    private suspend fun invokeWyzie(
        ...
        try {
            ...
            tryParseJson<ArrayList<WyzieSubtitle>>(res)?.map { subtitle ->
                // Menggunakan helper getLanguageNameFromCode yang ada di Utils.kt
            ...
            Log.e("Wyzie", "Error loading subtitles: ${e.message}")
        }
    }
    ...
    private suspend fun invokeVidrockSubs(
        ...
        try {
            ...
            Log.e("Vidrock", "Error loading subtitles: ${e.message}")
        }
    }
