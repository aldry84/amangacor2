...
open class AsianDrama : TmdbProvider() {
    ...
    data class KeywordResults(
        ...
        // FIX: Typo 'arrayLof' menjadi 'arrayListOf'
...
Data `AsianDrama.kt` (Perbaikan Typo)
...
open class AsianDrama : TmdbProvider() {
    ...
    /** Diadaptasi dari SoraStream */
    companion object {
        private const val tmdbAPI = "[https://api.themoviedb.org/3](https://api.themoviedb.org/3)"
        private const val apiKey = "b030404650f279792a8d3287232358e3" // Kunci API TMDB
    ...
    private fun getImageUrl(link: String?): String? {
        if (link == null) return null
        return if (link.startsWith("/")) "[https://image.tmdb.org/t/p/w500/$link](https://image.tmdb.org/t/p/w500/$link)" else link
    }

    private fun getOriImageUrl(link: String?): String? {
        if (link == null) return null
        return if (link.startsWith("/")) "[https://image.tmdb.org/t/p/original/$link](https://image.tmdb.org/t/p/original/$link)" else link
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        val data = try {
            if (url.startsWith("[https://www.themoviedb.org/](https://www.themoviedb.org/)")) {
                val segments = url.removeSuffix("/").split("/")
                val id = segments.lastOrNull()?.toIntOrNull()
                val type = when {
                    url.contains("/movie/") -> "movie"
        ...
        val trailer = res.videos?.results?.map { "[https://www.youtube.com/watch?v=$](https://www.youtube.com/watch?v=$){it.key}" }
