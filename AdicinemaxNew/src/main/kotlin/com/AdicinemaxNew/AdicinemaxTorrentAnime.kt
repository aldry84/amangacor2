...
import com.AdicinemaxNew.AdicinemaxNewAnime.LinkData
import okhttp3.MediaType.Companion.toMediaTypeOrNull
import okhttp3.RequestBody.Companion.toRequestBody
import org.json.JSONObject
import java.util.Calendar

open class AdicinemaxNewTorrentAnime : MainAPI() {
    override var name = "AdicinemaxNewTorrent-Anime"
    ...
    private suspend fun tmdbToAnimeId(title: String?, year: Int?, type: TvType): com.AdicinemaxNew.AniIds {
        if (title.isNullOrBlank()) return com.AdicinemaxNew.AniIds(null, null)
    ...
    """.trimIndent()
        ...
        val res = app.post(AdicinemaxNew.anilistAPI, requestBody = data)
            .parsedSafe<com.AdicinemaxNew.AniSearch>()
            ?.data
            ?.let { it.Page?.media ?: it.media }
            ?.firstOrNull()

        return com.AdicinemaxNew.AniIds(res?.id, res?.idMal)
    }
