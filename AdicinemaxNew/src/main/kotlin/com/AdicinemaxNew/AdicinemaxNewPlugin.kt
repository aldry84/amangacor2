...
class AdicinemaxNewPlugin : Plugin() {
    ...
    override fun load(context: Context) {
        ...
        val sharedPref = context.getSharedPreferences("AdicinemaxNew", Context.MODE_PRIVATE)
        
        // Main APIs untuk AdicinemaxNew
        val mainApis = listOf(
            AdicinemaxNew(sharedPref)
            // Tambahkan varian lainnya jika diperlukan:
            // AdicinemaxNewLite(sharedPref),
            // AdicinemaxNewTorrent(sharedPref),
            // AdicinemaxNewAnime(sharedPref),
            // AdicinemaxNewTorrentAnime(sharedPref)
        )
        ...
        Log.d("AdicinemaxNew", "SavedSet: $savedSet, DefaultEnabled: $defaultEnabled")
        Log.d("AdicinemaxNew", "Final enabled set: $enabledSet")

        for (api in mainApis) {
            if (enabledSet.contains(api.name)) {
                registerMainAPI(api)
                registeredMainApis.add(api)
                Log.d("AdicinemaxNew", "Registered plugin: ${api.name}")
            } else {
                Log.d("AdicinemaxNew", "Not enabled: ${api.name}")
            }
        }
        ...
        // Register semua extractor yang diperlukan untuk AdicinemaxNew
        ...
        // Settings untuk plugin AdicinemaxNew
        openSettings = { ctx ->
            val act = ctx as AppCompatActivity
            if (!act.isFinishing && !act.isDestroyed) {
                val frag = MainSettingsFragment(this, sharedPref)
                frag.show(act.supportFragmentManager, "AdicinemaxNewSettings")
            } else {
                Log.e("AdicinemaxNewPlugin", "Activity is not valid anymore, cannot show settings dialog")
            }
        }
    }
}
