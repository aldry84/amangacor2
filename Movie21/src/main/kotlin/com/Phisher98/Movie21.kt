...
class Movie21 : MainAPI() {
    ...
    // Endpoint Vidsrc VAPI
    private val vidsrcApi = "https://vidsrc.cc/vapi"

    // Helper untuk membuat format URL data: "TMDB_ID|tipe"
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        for (i in 0 until json.length()) {
            ...
            if (id == 0 || (mediaType != "movie" && mediaType != "tv")) continue // Hanya ambil movie/tv
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (mediaType == "movie") {
            ...
            // TvSeries Load Response - Membutuhkan Musim/Episode
            ...
            val seasons = if (seasonsArray != null) {
                (0 until seasonsArray.length()).mapNotNull { i ->
                    ...
                    if (seasonNumber >= 1 && episodeCount > 0) { // Hanya musim > 0
    ...
    // =============================
    // Diperlukan untuk Film (data = "TMDB_ID|movie") dan Serial TV (data = "TMDB_ID|S|E")
    override suspend fun loadLinks(
        ...
        if (parts.size == 2) { 
            // Film: data = "TMDB_ID|movie"
            val tmdbId = parts[0]
            vidsrcUrl = "$vidsrcApi/movie/$tmdbId"
        } else if (parts.size == 3) {
            // Serial TV: data = "TMDB_ID|SeasonNum|EpisodeNum"
            val (tmdbId, sNum, eNum) = parts
            vidsrcUrl = "$vidsrcApi/tv/$tmdbId/$sNum/$eNum"
        } else {
            return false
        }
        
        val jsonText = app.get(vidsrcUrl).text
        val json = JSONObject(jsonText)

        if (json.optBoolean("success")) {
            // Menggunakan ExtractorLink untuk link yang diekstrak
            val streams = json.optJSONArray("stream") ?: return true
            for (i in 0 until streams.length()) {
                val obj = streams.getJSONObject(i)
                val server = obj.optString("server", "Unknown")
                val link = obj.optString("url", "")
                
                // Panggil loadExtractor dengan URL link
                loadExtractor(link, "$vidsrcApi/$server", subtitleCallback, callback)
            }
        }
